...
export default function EditInvoicePage() {
  ...
  const fetchInvoiceAndData = async () => {
    try {
      ...
      fetchedInvoice.details.forEach((detail: any) => {
        const qty = Number(detail.currentQty);
        draftMap[detail.boqItemId] = qty;
        const mode = resolveBillingMode({
          billingMode: detail.boqItem?.billingMode,
          unit: detail.boqItem?.unit,
        });
        if (mode === 'LUMP_SUM_PROGRESS') {
          const contractQty =
            fetchedInvoice.contract?.items?.find((ci: any) => ci.boqItemId === detail.boqItemId)?.assignedQty ??
            detail.boqItem?.quantity ??
            1;
          const unitPrice =
            fetchedInvoice.contract?.items?.find((ci: any) => ci.boqItemId === detail.boqItemId)?.unitPrice ??
            detail.unitPrice;
          const pct =
            detail.currentProgressPercent ??
            qtyToProgressPercent(qty, contractQty);
          pctMap[detail.boqItemId] = pct;
          valMap[detail.boqItemId] = qty * unitPrice;
        } else {
          qtyMap[detail.boqItemId] = qty;
        }
      });
      setDraftQuantities(draftMap);
      setQtyInputs(qtyMap);
      setPercentInputs(pctMap);
      setValueInputs(valMap);

      const contractInvoices = fetchedInvoice.contract?.invoices || [];
  ...
  const buildExecutionPayload = () => {
    ...
    for (const item of boqItems) {
      ...
      if (mode === 'LUMP_SUM_PROGRESS') {
        ...
        if (pct > 0) rows.push({ boqItemId: item.id, currentPercent: pct, entryMode: 'PERCENT' });
        else if (val > 0) rows.push({ boqItemId: item.id, currentValue: val, entryMode: 'VALUE' });
      } else {
        const q = qtyInputs[item.id] || 0;
        if (q > 0) rows.push({ boqItemId: item.id, currentQty: q, entryMode: 'QTY' });
      }
    }
    return rows;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (payloadData.length === 0) {
      alert("الرجاء إدخال إنجاز واحد على الأقل (كمية أو نسبة أو قيمة).");
      return;
    }
                            ...
                            {isLumpSum ? (
                              <div className="flex flex-col gap-1 items-center">
                                <input
                                  ...
                                  className="w-14 h-7 bg-black/40 border border-slate-700/50 rounded-md text-center font-mono text-xs text-amber-400 focus:border-amber-500 focus:outline-none"
                                  placeholder="%"
                                />
                                <input
                                  type="number"
                                  ...
                                  disabled={isCompleted}
                                  className="w-20 h-7 bg-black/40 border border-slate-700/50 rounded-md text-center font-mono text-[10px] text-orange-300 focus:border-orange-500 focus:outline-none"
                                  placeholder="قيمة"
                                />
                              </div>
                            ) : (
