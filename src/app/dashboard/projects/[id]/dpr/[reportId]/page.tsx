...
export default function EditDPRPage() {
  ...
  useEffect(() => {
    const fetchReport = async () => {
      try {
        ...
        const res = await axios.get(`${API_BASE_URL}/v1/daily-reports/${reportId}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        const d = res.data;
        setReportDate(new Date(d.reportDate).toISOString().substring(0, 10));
        setWeather(d.weather || "مشمس");
        setTemperature(d.temperature ?? 35);
        setWorkPerformed(d.workPerformed || "");
        setSafetyNotes(d.safetyNotes || "");
        setLabors(d.labors?.length > 0 ? d.labors : [{ trade: "مهندس موقع", count: 1, hours: 8, notes: "" }]);
        setEquipments(d.equipments || []);
      } catch (err: any) {
        alert("فشل تحميل التقرير: " + (err.response?.data?.message || err.message));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.patch(`${API_BASE_URL}/v1/daily-reports/${reportId}`, payload, {
        headers: { Authorization: `Bearer ${token}` }
      });
