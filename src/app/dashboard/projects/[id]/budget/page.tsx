  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh]">
        ...
        <p className="text-slate-400 mt-6 text-xl font-bold animate-pulse">{t("common.loading")}</p>
      </div>
    );
  }
        ...
        <div className="flex items-center gap-5">
          ...
          <div>
            <h1 className="text-3xl font-extrabold text-white tracking-tight flex items-center gap-3 drop-shadow-sm">
              ...
              {t("project.budget.title")}
            </h1>
            <p className="text-slate-400 text-sm mt-2 font-medium flex items-center gap-2">
              <Building2 size={14} className="text-slate-500" />
              {t("project.budget.projectLabel")}: <span className="text-indigo-300 mx-1">{project.name}</span> ({project.code})
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="glass-dark border border-white/5 p-6 rounded-3xl relative overflow-hidden group">
           ...
           <p className="text-slate-400 font-bold text-sm mb-4 flex items-center gap-2 uppercase tracking-wide">
             <Target size={18} className="text-blue-400" /> {t("project.budget.revenue")}
           </p>
           <div className="space-y-4 relative z-10">
              <div>
                 <p className="text-[10px] text-slate-500 font-black tracking-widest uppercase">{t("project.budget.targetRevenue")}</p>
                 <p className="text-2xl font-mono font-black text-white">SAR {targetRevenue.toLocaleString()}</p>
              </div>
              <div className="h-px w-full bg-white/10" />
              <div>
                 <p className="text-[10px] text-slate-500 font-black tracking-widest uppercase">{t("project.budget.achievedRevenue")}</p>
                 <p className="text-lg font-mono font-bold text-blue-400">SAR {actualRevenue.toLocaleString()}</p>
                 <p className="text-xs text-blue-500/70 mt-1 font-mono">{targetRevenue > 0 ? ((actualRevenue/targetRevenue)*100).toFixed(1) : 0}% {t("project.budget.ofTarget")}</p>
              </div>
           </div>
        </div>
        ...
        <div className={`glass-dark border p-6 rounded-3xl relative overflow-hidden group transition-all ${isOverBudget ? 'border-rose-500/30' : 'border-emerald-500/20'}`}>
           ...
           <p className="text-slate-400 font-bold text-sm mb-4 flex items-center gap-2 uppercase tracking-wide">
             <Wallet size={18} className={isOverBudget ? "text-rose-400" : "text-emerald-400"} /> {t("project.budget.costAndBudget")}
           </p>
           <div className="space-y-4 relative z-10">
              <div>
                 <p className="text-[10px] text-slate-500 font-black tracking-widest uppercase">{t("project.budget.plannedBudget")}</p>
                 <p className="text-2xl font-mono font-black text-white">SAR {estimatedBudget.toLocaleString()}</p>
              </div>
              <div className="h-px w-full bg-white/10" />
              <div>
                 <p className="text-[10px] text-slate-500 font-black tracking-widest uppercase mb-1">{t("project.budget.actualSpend")}</p>
                 <div className="flex items-end gap-2">
                    ...
                    {isOverBudget && <AlertCircle size={14} className="text-rose-500 mb-1" title={t("project.budget.overBudget")} />}
                 </div>
                 ...
                 <p className="text-xs text-slate-500 mt-1.5 flex justify-between font-mono">
                     <span>{costSpentPercent.toFixed(1)}% {t("project.budget.spent")}</span>
                     <span className={variances.costVariance < 0 ? 'text-rose-400' : 'text-emerald-400'}>{t("project.budget.remaining")}: {Math.max(0, variances.costVariance).toLocaleString()}</span>
                 </p>
              </div>
           </div>
        </div>
        ...
        <div className="glass-dark border border-white/5 p-6 rounded-3xl relative overflow-hidden group">
           ...
           <p className="text-amber-400 font-bold text-sm mb-4 flex items-center gap-2 uppercase tracking-wide">
             <TrendingUp size={18} /> {t("project.budget.expectedProfit")}
           </p>
           <div className="space-y-4 relative z-10">
              <div>
                 <p className="text-[10px] text-slate-500 font-black tracking-widest uppercase">{t("project.budget.totalExpectedProfit")}</p>
              ...
              <div className="bg-amber-500/10 py-3 px-4 rounded-xl border border-amber-500/20">
                 <p className="text-[10px] text-amber-500/80 font-black tracking-widest uppercase mb-1">{t("project.budget.expectedProfitMargin")}</p>
                 <p className="text-xl font-mono font-black text-amber-400">{expectedProfitMargin.toFixed(2)}%</p>
                 <p className="text-[10px] text-amber-500/50 mt-1">{t("project.budget.profitCalcBasis")}</p>
              </div>
           </div>
        </div>
        ...
        <div className="glass-dark border border-white/5 p-6 rounded-3xl relative overflow-hidden group">
           ...
           <p className="text-indigo-400 font-bold text-sm mb-4 flex items-center gap-2 uppercase tracking-wide">
             <DollarSign size={18} /> {t("project.budget.currentProfit")}
           </p>
           <div className="space-y-4 relative z-10">
              <div>
                 <p className="text-[10px] text-slate-500 font-black tracking-widest uppercase">{t("project.budget.currentFinancialPosition")}</p>
              ...
              <div className={`py-3 px-4 rounded-xl border ${currentProfit >= 0 ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-rose-500/10 border-rose-500/20'}`}>
                 <p className={`text-[10px] font-black tracking-widest uppercase mb-1 ${currentProfit >= 0 ? 'text-emerald-500/80' : 'text-rose-500/80'}`}>{t("project.budget.currentProfitMargin")}</p>
                 <p className={`text-xl font-mono font-black ${currentProfit >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                    {currentProfitMargin.toFixed(2)}%
                 </p>
                 <p className={`text-[10px] mt-1 ${currentProfit >= 0 ? 'text-emerald-500/50' : 'text-rose-500/50'}`}>{t("project.budget.profitStabilityNote")}</p>
              </div>
           </div>
        </div>
         </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
         ...
         <div className="lg:col-span-1 glass-dark border border-white/5 rounded-3xl p-6 shadow-2xl relative">
            <h3 className="text-lg font-bold text-white mb-6 flex items-center gap-2 border-b border-white/5 pb-4">
              <PieChart size={20} className="text-purple-400" /> {t("project.budget.costAnalysis")}
            </h3>
            ...
            <div className="space-y-4">
               <div className="bg-slate-900/50 p-4 rounded-2xl border border-white/5 flex items-start gap-4">
                  ...
                  <div className="w-full">
                     <p className="text-slate-400 text-xs font-bold mb-1">{t("project.budget.materialsCost")}</p>
                     ...
                     <p className="text-[10px] text-slate-500 uppercase flex justify-between mt-1">
                        <span>{t("project.budget.approvedPOs")}</span>
               ...
               <div className="bg-slate-900/50 p-4 rounded-2xl border border-white/5 flex items-start gap-4">
                  ...
                  <div className="w-full">
                     <p className="text-slate-400 text-xs font-bold mb-1">{t("project.budget.subcontractorsCost")}</p>
                     <p className="text-lg font-mono font-black text-white">SAR {breakdown.subcontractorCosts.toLocaleString()}</p>
                     <p className="text-[10px] text-slate-500 uppercase flex justify-between mt-1">
                        <span>{t("project.budget.approvedSubcontractorInvoices")}</span>
               ...
               <div className="bg-slate-900/50 p-4 rounded-2xl border border-white/5 flex items-start gap-4">
                  ...
                  <div className="w-full">
                     <p className="text-slate-400 text-xs font-bold mb-1">{t("project.budget.pettyCash")}</p>
                     ...
                     <p className="text-[10px] text-slate-500 flex justify-between mt-1">
                        <span>{t("project.budget.pettyCashDesc")}</span>
         ...
         <div className="lg:col-span-2 glass-dark border border-white/5 rounded-3xl p-6 shadow-2xl overflow-hidden flex flex-col">
            <h3 className="text-lg font-bold text-white mb-6 flex items-center gap-2 border-b border-white/5 pb-4">
              <TrendingDown size={20} className="text-indigo-400" /> {t("project.budget.boqAnalysis")}
            </h3>
            ...
            <div className="flex-1 overflow-auto custom-scrollbar">
               <table className="w-full text-right text-sm">
                   <thead className="bg-slate-900/80 sticky top-0 z-10 backdrop-blur-md">
                      <tr>
                         <th className="px-4 py-3 text-slate-400 font-bold text-xs uppercase w-12 border-b border-white/5">{t("project.budget.boqCode")}</th>
                         <th className="px-4 py-3 text-slate-400 font-bold text-xs uppercase border-b border-white/5">{t("project.budget.boqDescription")}</th>
                         <th className="px-4 py-3 text-slate-400 font-bold text-xs uppercase border-b border-white/5 text-center">{t("project.budget.boqUnitCost")}</th>
                         <th className="px-4 py-3 text-slate-400 font-bold text-xs uppercase border-b border-white/5 text-center bg-indigo-500/5">{t("project.budget.boqUnitPrice")}</th>
                         <th className="px-4 py-3 text-slate-400 font-bold text-xs uppercase border-b border-white/5 text-left bg-indigo-500/10">{t("project.budget.boqProfitPerUnit")}</th>
                         <th className="px-4 py-3 text-slate-400 font-bold text-xs uppercase border-b border-white/5 text-left">{t("project.budget.boqExpectedValue")}</th>
                      </tr>
                   </thead>
